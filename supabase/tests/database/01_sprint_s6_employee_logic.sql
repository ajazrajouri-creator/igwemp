...
-- 8. Change Request Apply Returns Success
DO $$
DECLARE
  v_tenant_id uuid := '00000000-0000-4000-a000-000000000001';
  v_party_id uuid;
  ...
  v_result jsonb;
BEGIN
  INSERT INTO public.parties (tenant_id, party_type, display_name) VALUES (v_tenant_id, 'PERSON', 'Test Person Apply') RETURNING id INTO v_party_id;
  INSERT INTO public.person_parties (party_id, first_name) VALUES (v_party_id, 'Test');
  INSERT INTO public.employee_profiles (tenant_id, person_party_id, employee_code) VALUES (v_tenant_id, v_party_id, 'EMP_APPLY_1') RETURNING id INTO v_employee_id;
  
  INSERT INTO public.user_accounts (tenant_id, supabase_auth_id) VALUES (v_tenant_id, 'f0000000-0000-4000-a000-000000000001') RETURNING id INTO v_user_id;
...
  -- Set tenant context for the RPC
  PERFORM set_config('request.jwt.claims', format('{"tenant_id": "%s"}', v_tenant_id), true);

  v_result := public.apply_approved_employee_change_request(v_req_id);
  
  IF (v_result->>'success')::boolean != true THEN
    RAISE EXCEPTION 'apply function should return success';
  END IF;
END $$;

SELECT pass('apply_approved_employee_change_request returns success for non-auto-apply types');
